...
*Primary method: cross-validated d′(late vs early cue), tracked across training
days by region and cohort. The encoding-model ablation is retained as a
single-session validation. (Zhong et al. 2025 data.)*
...
flowchart TB
    ...
    **reward-encoding neurons** change across **training days**,
    ...
    subgraph PRIMARY["PRIMARY — d′(late vs early cue), across days"]
        ...
        (much larger than 1,000; capped at what a region has)`"]
    ...
    **mean d′** (how strongly); mean ± s.e.m. across mice,
    ...
    subgraph VALID["VALIDATION (one session) — encoding-model ablation"]
        direction TB
        V1["`**DESIGN MATRIX X** (time × variables)
        stimulus · position (bases) · velocity ·
        **reward & cue lag bases**`"]
        V1 --> V2["`**RIDGE FIT** per neuron on train trials`"]
        V2 --> V3["`**ABLATE REWARD BLOCK**, refit;
        ΔMSE on held-out = reward's unique value`"]
        V3 --> V4["`**PERMUTATION** on reward timing → p;
        candidate if ΔMSE > 0 and p < 0.05`"]
    end
    D1 --> V1
    D2 --> V1
    V4 -. confirms pipeline on 1 session .-> AGG
    ...
    CAV --> DEV
    DEV["`**DEVIATION NOTE**
    The proposal first specified the *ablation* for tracking. We
    switched the across-days tracking to **cross-validated d′**
    because it keys off the **cue** (fair across cohorts — water
    doesn't exist in unsupervised) and matches the paper's
    Fig. 4f–g. Ablation kept as the single-session validation.`"]
    ...
    classDef valid fill:#e2e8f0,stroke:#4a5568,color:#111
